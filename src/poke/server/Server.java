...
public class Server {
	...
	protected ChannelFactory publicCF, mgmtCF;
	...
	private void init(File cfg) {
		...
		publicCF = new NioServerSocketChannelFactory(Executors.newCachedThreadPool(), Executors.newCachedThreadPool());
	...
	private void createPublicBoot(int port) {
		...
		ServerBootstrap bs = new ServerBootstrap(publicCF);
	...
	private void createManagementBoot(int port) {
		...
		logger.info("Starting management communications, listening on port = " + port);
	}
	...
	private void startManagers() {
		...
		NetworkManager networkMgr = NetworkManager.getInstance(myId);
		...
		ElectionManager electionMgr = ElectionManager.getInstance(myId, votes);

		// create manager for accepting jobs
		JobManager jobMgr = JobManager.getInstance(myId);

		// establish nearest nodes and start receiving heartbeats
		HeartbeatManager heartbeatMgr = HeartbeatManager.getInstance(myId);
	...
	public void run() {
		...
		// initialize and start management services
		startManagers();
